...
#        data_utils.load_model_weights(generator_model, discriminator_model, DCGAN_model, name,classificator_model,zclass_model)
        ...
        loss4, acc4 = classificator_model.evaluate(Xsource_dataset, Ysource_dataset,batch_size=1024, verbose=0)                                                                                   
            ...
            for disc_it in range(disc_iterations):
                ...
                if model == 'wgan':
                    ...
                    if disc_type == "simple_disc":
                        ...
                        current_labels_gen =[np.zeros(X_disc_gen.shape[0]), virtual_real_labels] #(where I have the labels) instead of classifying the fake samples.That's because I have labels only for A domain
                ##############
