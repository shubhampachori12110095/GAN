...
def train(**kwargs):
    ...
    if dset == "mnistM":
        ...
        X_source_train,Y_source_train,X_source_test, Y_source_test,n_classes1 = data_utils.load_image_dataset(img_dim, image_dim_ordering,dset='bedrooms128small')
        X_dest_train,Y_dest_train,X_dest_test, Y_dest_test, n_classes2 = data_utils.load_image_dataset(img_dim, image_dim_ordering,dset='bedrooms128')
    elif dset == "bedrooms":
    ...
    generator_model = models.generator_deconv(noise_dim, img_source_dim,img_dest_dim, bn_mode,deterministic,pureGAN,inject_noise,wd)
    discriminator_model = models.discriminator_naive1(img_dest_dim, wd,inject_noise,model_name="discriminator_naive1")       
...
#    zclass_loss = zclass_model.fit([X_gen,X_source_train],[X_gen],batch_size=256,nb_epoch=10)
    ...
    ###classifier
    #####################
    if not( (dset == 'mnistM') or (dset == 'bedrooms') or (dset == 'bedrooms128')):
